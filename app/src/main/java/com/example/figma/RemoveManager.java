...
public class RemoveManager extends AppCompatActivity {
    ...
    private void showConfirmationDialog() {
        ...
        dialogMessage.setText("Are you sure you want to delete the manager?");
    ...
    private void deleteSelectedManagers() {
        SparseBooleanArray checkedItems = listView.getCheckedItemPositions();
        for (int i = 0; i < checkedItems.size(); i++) {
            int position = checkedItems.keyAt(i);
            if (checkedItems.valueAt(i)) {
                String name = names.get(position);
                new DeleteManagerTask().execute(name);
            }
        }
    }
    ...
    private class FetchNamesTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            try {
                URL url = new URL("https://house546.000webhostapp.com/fetchManager.php");
    ...
    private class DeleteManagerTask extends AsyncTask<String, Void, Boolean> {
        @Override
        protected Boolean doInBackground(String... params) {
            try {
                String name = params[0];
                String encodedName = URLEncoder.encode(name, "UTF-8");
                URL url = new URL("https://house546.000webhostapp.com/deleteManager.php?name=" + encodedName);
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                urlConnection.setRequestMethod("GET");
                int responseCode = urlConnection.getResponseCode();
                return responseCode == HttpURLConnection.HTTP_OK;
            } catch (Exception e) {
                Log.e("DeleteManagerTask", "Error deleting Manager", e);
                return false;
            }
        }
        ...
        protected void onPostExecute(Boolean success) {
            if (success) {
                Log.d("DeleteManagerTask", "Manager deleted successfully");
                // Refresh the list after deletion
                names.clear();
                new FetchNamesTask().execute();
            } else {
                Log.e("DeleteManagerTask", "Failed to delete manager");
            }
        }
    }
}
